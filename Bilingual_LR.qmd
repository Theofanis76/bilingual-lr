...
head(df_r, 2) # Check the first 2 rows of the dataset
```
...
```{r}
# a commonly used 80/20 train–test split
...
#### A recipe is defined to remove observations with missing values and convert height and weight to numeric format as part of the modeling workflow. In this analysis, missing values are handled by omitting incomplete cases using step_naomit().
#### The same workflow could be extended to include alternative preprocessing steps, such as imputation, normalization, or feature engineering, if required.
